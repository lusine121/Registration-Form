    ...
    if (count($news_error)=== 0 && $uploadOk !== 0) {
       ...
       var_dump($uploadOk);
        try {
           ...
           //$db = ('news_app', 'root', '', 'homework.local');
           //$db -> ('news', array('title' => '$title', 'description' => '$description', 'content' => '$content', 'date_of_creating' => '$date', 'image_path' => '$newname', 'category_id' => '$select')) -> result();

           //echo $db -> ('news', array('title' => '$title', 'description' => '$description', 'content' => '$content', 'date_of_creating' => '$date', 'image_path' => '$newname', 'category_id' => '$select')) -> result();
